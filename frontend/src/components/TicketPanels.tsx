      ...
      {logs.length === 0 ? (
        ...
        <div className="space-y-3 max-h-[450px] overflow-y-auto pr-1">
          {logs.map((w: any, i: number) => (
            <div
              key={w._id || i}
              className="rounded-xl bg-neutral-50/70 border border-neutral-200/60 p-3.5 space-y-1.5"
            >
              <div className="flex justify-between items-start gap-2">
                <div className="text-[11px] text-neutral-400 font-medium">
                  Ke: <span className="font-bold text-neutral-800">{w.recipientName}</span> ({w.phoneNumber})
                </div>
                <span className={`text-[9px] font-extrabold px-1.5 py-0.5 rounded ${
                  w.status === "SENT" || w.status === "DELIVERED" 
              ...
              <pre className="text-xs whitespace-pre-wrap font-sans text-neutral-600 leading-relaxed bg-white border border-neutral-100 rounded-lg p-2 font-medium">
                {w.messageContent}
              </pre>
              
              <div className="text-[10px] text-neutral-400 font-medium text-right">
                {format(new Date(w.sentAt || w.createdAt), "dd MMM HH:mm")}
              </div>
            </div>
          ))}
        </div>
